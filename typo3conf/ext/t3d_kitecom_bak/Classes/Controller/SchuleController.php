...
class SchuleController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	public function createAction(\kitecom\T3dKitecom\Domain\Model\Schule $newSchule) {
		...
		$this->flashMessageContainer->add('Your new School '.$newSpot->getName().' was created.');
	 ...
	 * action update
	 *
	 * @param \kitecom\T3dKitecom\Domain\Model\Schule $editschule
	 * @ignorevalidation $editschule
	 * @return void
	 */
	public function updateAction(\kitecom\T3dKitecom\Domain\Model\Schule $editschule) {
		...
		$this->schuleRepository->update($schulenew);
		$this->flashMessageContainer->add('Your Schule was updated.');
		$this->redirect('edit','Schule',Null,array('schule' => $schulenew));
	}	
