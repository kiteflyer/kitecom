...
//			}
	...
	public function showForm($valueCheck = array()) {
		...
		// Formular start.
		$content = '<form class="form-horizontal" name="' . $this->prefixId . '[' . $this->contentId . ']" action="' . $requestLink . '" method="post" enctype="multipart/form-data" id="' . $this->getFieldId('form') . '">';
		...
		foreach ($this->arrUsedFields as $fieldName) {
			...
			if ($this->feUsersTca['columns'][$fieldName]) {
				...
				// Label schreiben.
				$content .= '<label class="control-label" for="' . $this->getFieldId($fieldName) . '">' . $this->getLabel($fieldName) . '</label>';
				$contnet .='<div class="controls">';
			...
			if ($fieldName == self::specialfieldKeySubmit) {
				...
				$content .= '<input class="btn-orange pull-right" type="submit" value="' . $this->getLabel($fieldName . '_' . $this->conf['showtype'], FALSE) . '" id="' . $this->getFieldId($fieldName) . '" />';
				$content .= '</div>';
			...
			if ($fieldName == self::specialfieldKeyUserdelete && $this->conf['showtype'] == self::showtypeKeyEdit) {
				...
				$content .= '<label class="control-label" for="' . $this->getFieldId($fieldName) . '">' . $this->getLabel($fieldName) . '</label>';
				$contnet .='<div class="controls">';
				$content .= '<input class="'.$this->getErrorClass($fieldName, $valueCheck).'" type="checkbox" name="' . $this->getFieldName($fieldName) . '" value="1" id="' . $this->getFieldId($fieldName) . '" />';
				$content .= '</div>';
				//$content .= '</div>';
...
//				} else {
					...
					$content .= '<label class="control-label" for="' . $this->getFieldId($fieldName) . '">' . $this->getLabel($fieldName) . '</label>';
					$contnet .='<div class="controls">';
					$content .= '<input class="'.$this->getErrorClass($fieldName, $valueCheck).'" type="text" name="' . $this->getFieldName($fieldName) . '" value="" id="' . $this->getFieldId($fieldName) . '" />';
					$content .= '</div>';
					//$content .= '</div>';
					$iItem++;
//				}
			...
			if ($fieldName == self::specialfieldKeyPasswordconfirmation && $this->conf['showtype'] == self::showtypeKeyEdit) {
				...
				$content .= '<label class="control-label" for="' . $this->getFieldId($fieldName) . '">' . $this->getLabel($fieldName) . '</label>';
				$contnet .='<div class="controls">';
				$content .= '<input class="'.$this->getErrorClass($fieldName, $valueCheck).'" type="password" name="' . $this->getFieldName($fieldName) . '" value="" id="' . $this->getFieldId($fieldName) . '" />';
				$contnet .='</div>';
				//$content .= '</div>';
	...
	public function showInput($fieldName, $fieldConfig, $arrCurrentData, $disabledField = '', $valueCheck = array(), $iItem = 0) {
		...
		// Passwordfelder.
		if (in_array('password', $arrFieldConfigEval)) {
			$content .= '<div class="controls"><input class="'.$this->getErrorClass($fieldName, $valueCheck).'" type="password" name="' . $this->getFieldName($fieldName) . '" value=""' . $disabledField . ' id="' . $this->getFieldId($fieldName) . '" /></div>';	
			//$content .= '<div id="' . $this->getFieldId($fieldName, 'rep', 'wrapper') . '" class="' . $this->getFieldClasses($iItem, $fieldName, $fieldConfig['type'], $valueCheck) . '">';
			$content .= '<label class="control-label" for="' . $this->getFieldId($fieldName, 'rep') . '">' . $this->getLabel($fieldName . '_rep', false) . $this->isRequiredField($fieldName) . '</label>';
			$content .= '<div class="controls"><input class="'.$this->getErrorClass($fieldName, $valueCheck).'"  type="password" name="' . $this->prefixId . '[' . $this->contentId . '][' . $fieldName . '_rep]" value=""' . $disabledField . ' id="' . $this->getFieldId($fieldName, 'rep') . '" /></div>';
		...
		$content .= '<input class="'.$this->getErrorClass($fieldName, $valueCheck).'" type="text" name="' . $this->getFieldName($fieldName) . '" value="' . $arrCurrentData[$fieldName] . '"' . $disabledField . ' id="' . $this->getFieldId($fieldName) . '"' . $additionalAttributes . ' />';
		$content .= '</div>';
		return $content;
	}
	...
	public function showCheck($fieldName, $fieldConfig, $arrCurrentData, $disabledField = '') {
		...
		if (count($fieldConfig['items']) > 1) {
			...
			foreach (array_values($fieldConfig['items']) as $key => $checkItem) {
				...
				$contnet .='</div>';
				$content .= '<label class="control-label" for="' . $this->getFieldId($fieldName, 'item', $i) . '">' . $this->getLabel($checkItems[$key][0], false) . '</label>';
				//$content .= '</div>';
	...
	public function showRadio($fieldName, $fieldConfig, $arrCurrentData, $disabledField = '') {
		...
		foreach (array_values($fieldConfig['items']) as $radioItem) {
			...
			//$content .= '<div id="' . $this->getFieldId($fieldName, 'item', $i, 'wrapper') . '" class="item item-' . $i . '">';
			$content .= '<label class="radio-inline" for="' . $this->getFieldId($fieldName, 'item', $i) . '">';
	...
	public function showSelect($fieldName, $fieldConfig, $arrCurrentData, $disabledField = '') {
		...
		if ($fieldConfig['renderMode'] == 'checkbox') {
			...
			$content .= '<div class="controls">';
			$content .= '<select class="'.$this->getErrorClass($fieldName, $valueCheck).'" name="' . $this->getFieldName($fieldName) . '' . (($multiple) ? '[]' : '') . '"' . $multiple . $disabledField . ' id="' . $this->getFieldId($fieldName) . '">';
			$content .= $optionlist;
			$content .= '</select></div>';
		}
...
//		$content .= ($showInput) ? '<input type="text" name="' . $this->getFieldName($fieldName) . '" value="" id="' . $this->getFieldId($fieldName) . '" />' : '';
	...
	public function getErrorClass($fieldName, $valueCheck) {
		...
		if (($errorType = $this->getErrorType($fieldName, $valueCheck))) {
			$class = 'f3-form-error error-' . $errorType;
		}
