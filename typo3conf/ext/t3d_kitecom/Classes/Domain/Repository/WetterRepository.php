 ...
 *
 */
class WetterRepository extends \TYPO3\CMS\Extbase\Persistence\Repository {
...
/**
    ...
    private function getRequest($type_of_request = 'local')
    {
        ...
        if (self::$caching == false) {
            ...
            if ($doc === FALSE) {
                return('Data could not be retrieved for location: <strong>' . self::$city . '-' . self::$country . '</strong>.');
            }
            ...
            // Cache file
            $cache_file = 'typo3temp/weathercache/cached-' . (substr($key, 0, 8) . '-' . $city . $country) . '.xml';
    ...
    public function getLocalWeather()
    {
        ...
        $new_doc->weather_now = array(
            ...
            "weatherTemp" => (self::$us_metric ? $current->temp_F : $current->temp_C) . '°C',
        ...
        foreach ($doc->weather as $weather) {
            ...
            //$ndate = mktime(0,0,0,date("m"),date("d")+1,date("Y"));
            //$ndate = strtotime($weather->date)+83400;
            ...
            $weather_max = (self::$us_metric ? $weather->tempMaxF : $weather->tempMaxC) . '°C';
            $weather_min = (self::$us_metric ? $weather->tempMinF : $weather->tempMinC) . '°C';
